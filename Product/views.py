...
class ProductListView(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if brand:
            ...
            queryset = ProductVersion.objects.filter(Q(product__category__name=category) | Q(
                product__category__parent__name=category) | Q(product__category__parent__parent__name=category))
        else:
            queryset = ProductVersion.objects.all()
        return queryset
...
class ProductDetailView(DetailView, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['relateds'] = ProductVersion.objects.exclude(id=version.id).filter(Q(product__id=version.id) | Q(
            product__category__name=version.product.category.name) | Q(color__name=version.color.name))[:6]
